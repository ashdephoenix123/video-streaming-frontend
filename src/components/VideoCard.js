...
const menuItems = [
  { icon: Trash2, label: "Remove", id: "remove" },
  { icon: Timer, label: "Add to Watch Later", id: "watch-later" },
];

const VideoCard = ({ vid, user }) => {
  const [show, setShow] = useState(false);

  const itemAction = (action) => {
    console.log(action);
    // Perform remvoe from liked videos or save to watch later
    // vid._id
    setShow(false);
  };
  ...
  return (
    ...
    >
      <div className="w-80 relative h-48">
        {vid.hlsUrl ? (
      ...
      <div className="p-4 flex gap-8 flex-1">
        <div className="flex-1 space-y-1">
          ...
          <div className="flex items-center gap-1.5 mt-2">
            <Image
              src={user.userAvatar || "/default-user.jpg"}
